...
type BackToProjectsPillProps = {
  /** Scroll threshold (px) after which the pill appears. Default 320. */
  threshold?: number;
};

/**
 * Sticky "Back to projects" pill that floats at the top-left of the
 * viewport on project detail pages.
 *
 * Behaviour:
 *  - Hidden when the user is at the top of the page (hero is visible,
 *    breadcrumb in the fixed nav already provides navigation context).
 *  - Fades + slides in once the user scrolls past `threshold` px,
 *    giving them a persistent escape hatch back to /projects without
 *    having to scroll all the way up to the breadcrumb.
 ...
 *  - Hover: emerald accent ring + arrow nudges left.
 *  - Mobile: pill shrinks to icon-only on very narrow viewports.
 *
 * This complements the breadcrumb nav (round 3) and the back-to-top
 * button (round 5) — together they form a complete wayfinding system.
 */
export function BackToProjectsPill({ threshold = 320 }: BackToProjectsPillProps) {
    ...
    <AnimatePresence>
      {visible && (
        ...
        >
          ...
          <span className="text-xs font-medium hidden sm:inline">All projects</span>
          <span className="text-xs font-medium sm:hidden">Projects</span>
          <FolderOpen className="w-3.5 h-3.5 text-foreground/40 group-hover:text-emerald-500/70 transition-colors hidden sm:block" />
        </motion.button>
      )}
    </AnimatePresence>
  );
