    ...
    <div className="flex items-center gap-2 flex-wrap">
      {socials.map((social, i) => (
        <motion.a
          ...
          className="group flex items-center gap-2 px-3.5 py-2.5 sm:py-2 rounded-full border border-outline-4 bg-surface-2 hover:bg-surface-4 hover:border-emerald-500/30 hover:shadow-sm hover:shadow-emerald-500/10 transition-all duration-200 min-h-[44px] sm:min-h-0"
          aria-label={social.name}
        >
          <span className="text-foreground/85 group-hover:text-foreground transition-colors">
            {social.icon}
          </span>
          <span className="text-[11px] font-medium text-foreground/75 group-hover:text-foreground transition-colors">
            {social.name}
          </span>
        </motion.a>
      ))}
